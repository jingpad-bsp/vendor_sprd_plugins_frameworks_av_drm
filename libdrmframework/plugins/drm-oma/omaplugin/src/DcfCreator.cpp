...
DcfCreator::DcfCreator(char* file, char* transKey, char* contentType, char* contentUri)
    ...
    if ((fd = open(file, O_WRONLY | O_CREAT | O_TRUNC, 0777)) == -1) {
        ALOGE("DcfCreator::DcfCreator: cant open file %s, %s", file, strerror(errno));
    }
...
DcfCreator::DcfCreator(char* /*file*/, char* transKey, char* contentType, char* contentUri)
    : encryptContext(NULL),
      ...
      contentDescription(NULL),
      contentVendor(NULL) {
    encryptionMethod = (char*)DCF_DEFAULT_ENCRYPTION_METHOD;
...
bool DcfCreator::setKey(char* transKey) {
    ...
    ALOGD("DcfCreator::setKey: trans key: %s", transKey);
    ALOGD("DcfCreator::setKey: origKey %s, origKeyLen: %d", origKey, origKeyLength);
...
char* DcfCreator::convert(char* buffer, ssize_t bytesReaded, ssize_t* bytesConverted) {
    ...
    if (!encryptContext) {
        ...
        EVP_EncryptUpdate(encryptContext, (unsigned char*)(cipherText + 16), &cLen, (unsigned char*)buffer,
                          bytesReaded);
...
bool DcfCreator::save() {
    ...
    int dOffset = lseek(fd, dataLenOffset, SEEK_SET);
    if (dOffset == -1) {
        return false;
    }
...
bool DcfCreator::writeHeaders() {
    ...
    // content type len
    if (!contentType) {
        ALOGE("DcfCreator::contentType unset");
        return false;
    }
    ...
    // content uri len
    if (!contentUri) {
        ALOGE("DcfCreator::contentUri unset");
        return false;
    }
    ...
    String8 headers;
    if (!encryptionMethod) {
        ALOGE("DcfCreator::encryptionMethod unset");
        return false;
    }
    ...
    ALOGD("DcfCreator::writeHeaders: headers: %s, headersLen: %d", headers.string(), headersLen);
    // uintvar can be encoded into 5 octets at most
    for (int i = 4; i >= 0; --i) {
        // ssize_t is typically unsigned, thus logical right shift
        // would work
...
bool DcfCreator::convertHeaders() {
    ...
    ALOGD("DcfCreator::convertHeaders: headers: %s, headersLen: %d", head.string(), headersLen);
    // uintvar can be encoded into 5 octets at most
    for (int i = 4; i >= 0; --i) {
        // ssize_t is typically unsigned, thus logical right shift
        // would work
    ...
    unsigned char tmp[] = {0x80, 0x80, 0x80, 0x80, 0x00};
    headers.append((const char*)tmp, 5);
    // headers
    headers.append(head);
