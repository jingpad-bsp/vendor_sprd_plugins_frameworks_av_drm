...
#define LOG_TAG "DRM_RightsParser"
...
RightsParser::RightsParser(const DrmRights& rights) : mimeType(rights.getMimeType()), state(UNKNOWN) {
    ...
    if (0 == origLength) {
        ALOGE("RightsParser construct error! date length is zero, maybe open rights-object file failed!");
...
void RightsParser::DumpConstrains() {

    for (int i = 0; i < ACTION_NUM; ++i) {
        ALOGD("constraint[%d], %lld, %lld, %lld, %lld, %lld", i, constraints[i][0],constraints[i][1],constraints[i][2],constraints[i][3],constraints[i][4]);
    }
}

bool RightsParser::parse() {
    ALOGD("parse rights, current state:%d", state);
    ...
    if (mimeType == String8(MIMETYPE_RO)) {
        ...
        ALOGE("parse, unknown mimetype: %s", mimeType.string());
        ret = false;
    }
    if (ret) {
        state = OK;
        ...
        if (key.isEmpty()) {
            ...
            RightsManager rm;
            key = rm.getFakeKey();
            ALOGD("parse, no key, fake key = %s", key.string());
        }
    } else {
        ALOGE("parse, ERROR");
        state = ERROR;
...
bool RightsParser::parseXML() {
    ...
    if (myDocument->Error()) {
        ALOGE("parseXML: parse xml fail: %s", myDocument->ErrorDesc());
        delete myDocument;
        return false;
    }
    ...
    ALOGD("parseXML: origUid = %s", origUid);
    if (!strstr(origUid, "cid:")) {
        ALOGE("parseXML: uid wrong: %s", uid.string());
        delete myDocument;
        return false;
    }
    ...
    if (permissionNode) {
        SUNWAY_NOISY(ALOGD("parseXML: got permissionNode"));
        for (unsigned int i = 0; i < sizeof(actionsName) / sizeof(char*); ++i) {
            TiXmlNode* actionNode = permissionNode->FirstChild(actionsName[i]);
            SUNWAY_NOISY(ALOGD("parseXML: checking for %s", actionsName[i]));
            if (actionNode) {
                SUNWAY_NOISY(ALOGD("parseXML: got node for %s", actionsName[i]));
                TiXmlNode* tmpNode = actionNode->FirstChild(RO_ELEMENT_CONSTRAINT);
                if (tmpNode) {
                    for (unsigned int j = 0; j < sizeof(constraintsName) / sizeof(char*); ++j) {
                        ...
                        if (constraintNode) {
                            ...
                            ALOGD("parseXML: got node for %s = %s", constraintsName[j], tmpValue);
...
int RightsParser::checkRightsStatusForAction(int action) {
    ALOGD("checkRightsStatusForAction: %d", action);

    int64_t* constraintsForAction = constraints[action];
    if (state == NOT_ACQUIRED) {
        ALOGE("checkRightsStatusForAction result: not acquired");
        return RightsStatus::RIGHTS_NOT_ACQUIRED;
    }
    if (state != OK) {
        ALOGE("checkRightsStatusForAction: parse state error");
    ...
    ALOGD("checkRightsStatusForAction: count:%lld", constraintsForAction[COUNT]);
    ...
    if (intervalLimited) {
        if (constraintsForAction[INTERVAL_START] == UNSET) {
            ...
            if (0xffffffffffffffff - (uint64_t)constraintsForAction[INTERVAL_START] <=
                (uint64_t)constraintsForAction[INTERVAL]) {
                return RightsStatus::RIGHTS_VALID;
            } else {
                uint64_t cTime = (uint64_t)getCurrentTime();
                ALOGD("RightsParser::INTERVAL_START: %lld, INTERVAL: %llu, cTime: %llu",
                                   constraintsForAction[INTERVAL_START], constraintsForAction[INTERVAL], cTime);
    ...
    if (timeLimited) {
        ...
        ALOGD(
            "checkRightsStatusForAction, currentTime:%lld, "
            "start:%llu, end:%llu",
    ...
    ALOGD("checkRightsStatusForAction, can't find any constraint, invalid");
...
DrmConstraints* RightsParser::getConstraintsForAction(int action) {
    ...
    if (constraintsForAction[COUNT] != UNSET) {
        ...
        ret->put(&(DrmConstraints::REMAINING_REPEAT_COUNT),
                 String8::format("%lld", constraintsForAction[COUNT]).string());
    }

    if (constraintsForAction[INTERVAL] != UNSET) {
        if (constraintsForAction[INTERVAL_START] != UNSET) {
            ret->put(&(DrmConstraints::EXTENDED_METADATA),
                     String8::format("%lld", (constraintsForAction[INTERVAL_START]  - ntp_property_get())).string());
        }
        ret->put(&(DrmConstraints::LICENSE_AVAILABLE_TIME),
                 String8::format("%lld", constraintsForAction[INTERVAL]).string());
    }

    if (constraintsForAction[DATETIME_START] != UNSET) {
        ret->put(&(DrmConstraints::LICENSE_START_TIME),
                 String8::format("%lld", constraintsForAction[DATETIME_START]).string());
    }

    if (constraintsForAction[DATETIME_END] != UNSET) {
        ret->put(&(DrmConstraints::LICENSE_EXPIRY_TIME),
                 String8::format("%lld", constraintsForAction[DATETIME_END]).string());
    }

    ALOGD("getConstraintsForAction: counts = %lld, datetime_start = %lld, datetime_end = %lld, "
        "interval_start = %lld, interval = %lld",
        constraintsForAction[COUNT], constraintsForAction[DATETIME_START], constraintsForAction[DATETIME_END],
        constraintsForAction[INTERVAL_START], constraintsForAction[INTERVAL]);
...
int64_t RightsParser::parseRELInterval(const char* interval) {
    ...
    if (ret != 6) {
        SUNWAY_NOISY(ALOGE("parseRELInterval: wrong interval format: %s", interval));
        return EXPIRED;
    }
    int64_t relInterval =
        second + minute * 60 + hour * 3600ll + day * 24 * 3600ll + month * 31 * 24 * 3600ll + year * 366 * 24 * 3600ll;
    SUNWAY_NOISY(ALOGD("parseRELInterval: interval = %llu", relInterval));
...
bool RightsParser::parseWBXML() {
    ...
    if (!plainXml) {
        ALOGE("parseWBXML convert error");
        return false;
    }
    free(data);
    data = plainXml;
