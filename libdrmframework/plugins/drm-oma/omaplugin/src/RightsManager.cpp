#define LOG_TAG "DRM_RightsManager"
...
RightsManager::RightsManager() {
    ...
    if (rc != SQLITE_OK) {
        ALOGE(" database error: %s", sqlite3_errmsg(db));
        return;
    }
...
String8 RightsManager::getFakeKey() {
    if (sFakeKey.length()) return sFakeKey;
        ALOGD("getFakeKey: sFakeKey is null, try 2 query db");
...
String8 RightsManager::getFakeUid() {
    if (sFakeUid.length()) return sFakeUid;
        ALOGD("getFakeUid: sFakeUid is null, try 2 query db");

    if (!db) {
        ALOGD("getFakeUid: db is null");
        return sFakeUid;
    }
    ...
    if (rc != SQLITE_OK) {
        ALOGD("getFakeUid: failed: %s", sqlite3_errmsg(db));
        return sFakeUid;
    }
    rc = sqlite3_step(stmt);
    if (rc == SQLITE_ROW)
...
DrmConstraints* RightsManager::getConstraints(String8& uid, int action) {
    ...
    ALOGD("getConstraints, uid: %s, action: %d", uid.string(), action);

    RightsParser parser = query(uid);
    if (! parser.parse()) {
        ALOGE("getConstraints, parse right error! return defalut constraints");
        return new DrmConstraints();
    }
...
RightsParser RightsManager::query(const String8& uid) {
    ALOGD("query by uid %s", uid.string());
    RightsParser ret = RightsParser(RightsParser::ERROR);
    if (!db) {
        ALOGE("query failed, db is null");
        return ret;
    }
    ...
    if (rc != SQLITE_OK) {
        ALOGE("query: failed: %s", sqlite3_errmsg(db));
        return ret;
    }
    ...
    if (rc == SQLITE_ROW) {
        ...
        ALOGD("query: version: %s, uid: %s, key: %s", ret.version.string(), ret.uid.string(), ret.key.string());
        for (int i=0; i<ACTION_NUM; ++i) {
            for (int j=0; j<CONSTRAINT_NUM; ++j) {
                ret.constraints[i][j]=sqlite3_column_int64(stmt,4+i*CONSTRAINT_NUM+j);
            }
        }
        ALOGD("query: ok: %s", ret.uid.string());
    } else {
        ALOGE("query: NOT_ACQUIRED, err code: %d", rc);
...
sqlite3* RightsManager::openDatabase() {
    ...
    if (rc) {
        ALOGE("RightsManager::openDatabase: failed: %s", sqlite3_errmsg(db));
        return NULL;
    }

    const char* sql = "create table rights(\
...
bool RightsManager::insertOrUpdate(RightsParser& parser) {
    ...
    ALOGD("insertOrUpdate, sql: %s", sql.string());
    char * zerr;
    int rc = sqlite3_exec(db,sql.string(),0,0,&zerr);


    if (rc != SQLITE_OK) {
        ALOGE("insertOrUpdate, exec sql failed!");
        return false;
    }

    ALOGD("insertOrUpdate success");
